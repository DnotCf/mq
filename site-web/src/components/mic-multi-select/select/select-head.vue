<template>
    <div @click="onHeaderClick" :class="headCls">
        ...
        <template v-if="maxTagCount === undefined">
          <div
        ...
        <template v-else>
          <div
              class="ivu-tag ivu-tag-checked"
              v-for="(item, index) in selectedMultiple"
              v-show="index < maxTagCount && selectedMultiple.length <= maxTagCount">
              <span class="ivu-tag-text">{{ item.tag !== undefined ? item.tag : item.label }}</span>
              <Icon type="ios-close" @click.native.stop="removeTag(item)"></Icon>
          </div>
        </template>
        
        <div class="ivu-tag ivu-tag-checked" v-show="maxTagCount !== undefined && selectedMultiple.length > maxTagCount">
            <span class="ivu-tag-text ivu-select-max-tag">
                ...
                <template v-else>已选择<span class="ivu-select-max-number"> {{ selectedMultiple.length }}</span> 项</template>
            </span>
        </div>
        <span
...
<style lang="less">
  .ivu-select-multiple .ivu-tag span:not(.ivu-select-max-tag).ivu-select-max-number {
    display: inline;
    margin: 0;
    padding: 0 5px;
    color: #1A8AF7;
